...
public class UserController {
    ...
    public UserDTO queryUserLogin(@RequestBody UserDTO login) {
        UserDTO userDTO = userService.queryUser(login.getNif());
        if(userDTO != null){
            if(userService.verifyPassword(userDTO, login.getPassword())){
                return userDTO;
            }
        }
        return null;
